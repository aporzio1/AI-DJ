...
**Last updated:** 2026-04-26 — **K23 logged from K22 spike on bundled neural TTS.** sherpa-onnx + Piper VITS direct-link path is **Hold** for App Store distribution: the Piper runtime statically links espeak-ng (GPL-3) and requires its phoneme data at runtime, which would propagate GPL-3 to the closed-source main Patter binary and conflict with Apple's Developer Program License Agreement (the well-known VLC iOS situation). Two App Store precedents — "Piper - Neural TTS" (id 6759636010) and the eSpeak NG iOS app — both ship Piper voices via `AVSpeechSynthesisProviderAudioUnit` extensions, isolating GPL code in a separate bundle while keeping the host app proprietary. Tracker recommends **Path X** (commit the in-flight working tree + improve onboarding voice UX, ~1 day) as the next move and demotes "bundled TTS" to Path Y (system-voice extension, scoped later). OpenAI default-prevention requirement already satisfied in code (`SettingsViewModel.ttsProvider = .system` at declaration). Working tree is dirty: 11 modified files + 1 untracked, branch diverged 1↔1 with `origin1/main` — commit before further work. Earlier today: tracker purge of pending Spotify items per K21 lock; app rename "AI DJ" → "Patter" landed. N1.3 (article-link button) still paused mid-implementation — see §3.
...
### Working tree is dirty as of 2026-04-26 — commit before further work

**11 modified files + 1 untracked, branch diverged 1↔1 with `origin1/main`:**

```
 M Patter/App/RootView.swift
 M Patter/Resources/Info.plist
 M Patter/Resources/Patter.entitlements
 M Patter/Services/PlaybackCoordinator.swift
 M Patter/Services/Producer.swift
 M Patter/Services/SystemDJVoice.swift            (+120 LOC: chunked render, timeout, quality preference)
 M Patter/Utilities/Keychain.swift
 M Patter/ViewModels/SettingsViewModel.swift      (Kokoro iOS-26 auto-downgrade — see K6 path c)
 M Patter/Views/PreferencesWizardView.swift
 M Patter/Views/SettingsView.swift
 M project.yml
?? Patter/Models/SuggestedRSSFeed.swift
```

**Risk:** stacking new architecture (Path X wizard work, or anything else) on this WIP is unsafe — both because the diverged-branch state could lose work on a wrong rebase, and because the SystemDJVoice quality work + Kokoro iOS-26 downgrade are exactly the cheap fixes Path X depends on. **Commit and reconcile with `origin1/main` before any further edits.**

### Path X — finish in-flight System voice upgrade + onboarding voice UX *(recommended next, ~1 day)*

Replaces the original "bundled TTS" plan that was on deck for a spike (now **Hold** per K23 — see §6). Two-commit shape:

1. **Commit the in-flight working tree.** SystemDJVoice quality preference + sentence-chunked rendering + per-script timeout (~120 LOC), plus the `SettingsViewModel` iOS-26 `.kokoro → .system` auto-downgrade (already implemented in the dirty tree, lines 230–237). Closes Backlog #6 path (c) for iOS Kokoro compile hang.
2. **Add an iOS premium-voice nudge to the onboarding wizard.** Detect whether the device has any premium-quality English voice installed; if not, deep-link to Settings → Accessibility → Spoken Content → Voices with copy explaining why. macOS already has this pattern in `84cf31c` (System Settings → Spoken Content); iOS doesn't yet. Wizard runs once at first launch — the nudge is one screen, not a recurring prompt.

**Why this is the right next move:** solves the actual user-facing problem ("robotic compact voice on a fresh install") with zero new dependencies, zero GPL exposure, zero bundle bloat, and the Apple Premium voice quality has narrowed the gap to Piper-medium materially over the last 24 months (iOS 17 made Premium voices on-device + free; iOS 18+ kept improving them). Closes Backlog #6 cleanly without spending 5–10 days on an Audio Unit Extension.
...
| 6 | **Kokoro iOS 26 CoreML compile hang** | Path (c) **in flight** in the uncommitted working tree (`SettingsViewModel` lines 230–237: auto-downgrade `.kokoro → .system` on iOS 26). Pairs with the SystemDJVoice premium-quality preference + chunked render also in the dirty tree. Lands as part of Path X (see §3). Original investigation paths kept for record: (a) file FluidAudio upstream issue, (b) try different Kokoro model variant, (c) default iOS new installs to System Voice [shipping], (d) gate Kokoro behind a "Download model" button. | — |
| 7 | **Path Y — Patter-branded system voice via `AVSpeechSynthesisProviderAudioUnit` extension** | Re-scoped from the original "bundled TTS" plan after K23 found GPL-3 contamination blocks direct linking. Ship a standalone Audio Unit Application Extension that registers a high-quality neural voice (Piper VITS or similar) as a system voice. Extension binary is GPL-3 (clean isolation, links libespeak-ng there); main Patter binary stays MIT/proprietary and talks to the extension via XPC, same pattern as the eSpeak NG iOS app. Voice becomes available to VoiceOver, Reader, Patter's DJ, and any other app — bigger product surface than DJ-only. Est. 5–10 days. **Only legally-clean path to ship Piper on Apple's App Stores.** Two App Store precedents: "Piper - Neural TTS" (id 6759636010) and eSpeak NG iOS. Defer until Path X demonstrates Apple Premium isn't enough. | (TBD, scope as separate plan doc — must include K22 Constraints & Direction-of-Travel section, see K23 for primary sources) |
