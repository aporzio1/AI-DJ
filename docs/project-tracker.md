...
**Last updated:** 2026-04-20 (Library cache — Go: stale-while-revalidate on UserDefaults, device-local only, silent-stale on failure, single commit)
...
| Kokoro download indicator — Phase 1: icon + spinner in MiniPlayerBar | Option C: when Kokoro's `ensureInitialized` is in flight, show `arrow.down.circle` + `ProgressView().controlSize(.mini)` in the top row, mirroring the existing `waveform` "DJ speaking" indicator — no new row, no layout shift. New `@MainActor @Observable KokoroDownloadState` singleton owned by the router; `KokoroSynthesizer.ensureInitialized` flips `isDownloading` at entry and clears it in a `defer` (covers throw + success + background suspend/resume). `NowPlayingViewModel` exposes `isDownloadingVoiceModel: Bool` passthrough. When `currentItem == nil`, swap subtitle to "Downloading DJ voice…"; during playback keep title and show only the icon. Same state object feeds the existing Settings→Download Model button so both call sites stay consistent with `760e17e`. No percentage — FluidAudio v0.13.5 doesn't expose progress and directory-size polling lies during the CoreML final-compile stall. Single commit, ~45 min. | Scope decided 2026-04-20; ready to implement |
| Library cache — stale-while-revalidate + pull-to-refresh | User noticed Recently Played / Made for You regenerate on every cold launch, sometimes get cancelled (-999) mid-flight racing with iCloud identity checks. Hybrid B+C: (1) UserDefaults-backed disk cache, keys `library.recentlyPlayed.v1` / `library.recommendations.v1`, wrapper `{ fetchedAt: Date, items: [LibraryItem] }`, 30 min TTL. (2) On LibraryView `.task`, read cache synchronously → render instantly → kick background refresh if older than TTL, else skip (prevents tab-rebuild network thrash). (3) Pull-to-refresh on the List forces re-fetch. (4) Silent-stale on failure — log the error, leave cache on screen; only show `ContentUnavailableView` + retry button when cache is empty AND fetch fails (true offline-first-launch). (5) Cache is **device-local only** — NOT routed through `iCloudSyncService`; Recently Played is inherently per-device and Recommendations are server-personalized, syncing adds conflict surface for zero UX gain. Scope additions: add `Codable` to `PlaylistInfo` (currently `Identifiable, Sendable, Hashable` in `MusicKitServiceProtocol.swift`), `AlbumInfo`, `StationInfo`, and `LibraryItem` (Track already has Codable). New `LibraryCacheService` struct with `load(key:)` / `save(items:, key:)` / `isStale(key:, ttl:)`. No invalidation on track-finish — TTL covers it, and MusicKit's Recently Played is global across devices so optimistic prepend can drift. Single commit, ~75–90 min. | Scope decided 2026-04-20; ready to implement |
...
| Kokoro download indicator — Phase 2: real % progress | Replace the indeterminate spinner with a deterministic `ProgressView(value:)`. Blocked on EITHER: (a) FluidAudio upstream adds a download-progress callback to `KokoroTtsManager.initialize()`, or (b) we swap FluidAudio's internal HF downloader for our own `URLSession` + `.downloadProgress` pipe. (b) is out of scope for the MVP. | Backlog — revisit quarterly; file FluidAudio issue if one doesn't already exist |
...
| K9 | FluidAudio exposes no download-progress callback | low | `KokoroTtsManager.initialize()` is opaque — we can only observe running vs. returned. Directory-size polling against ~300 MB is brittle (CoreML compile stall, g2p+kokoro split, resume chunks). Keep indeterminate spinner until FluidAudio publishes a progress API or we ship our own downloader. |
